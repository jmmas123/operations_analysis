...
# Refactored main in operational_data.py

def load_and_process_data(start_date_str, end_date_str, analysis_type, entity_id=None):
    """
    Main processing function for operational data.

    Args:
        start_date_str (str): Start date as a string (dd/mm/yy or dd-mm-yy).
        end_date_str (str): End date as a string (dd/mm/yy or dd-mm-yy).
        analysis_type (str): 'C' for Client, 'W' for Warehouse.
        entity_id (str): Client or Warehouse ID for filtering.

    Returns:
        dict: Processed dataframes and results for dashboard use.
    """
    # Convert dates
    ...
    (wl_ingresos, rpshd_despachos, rpsdt_productos,
     ...
     supplier_info, ctcentro_table, producto_modelos, dispatched_inventory,
     inventario_sin_filtro) = load_data()

    # Convert 'descrip' and 'idcontacto' to strings
    supplier_info['descrip'] = supplier_info['descrip'].astype(str).str.strip()
    supplier_info['idcontacto'] = supplier_info['idcontacto'].astype(str).str.strip()

    # Filter by Client or Warehouse
    if analysis_type == 'C':
        dataframes_to_filter = [wl_ingresos, rpshd_despachos, rpsdt_productos, registro_ingresos,
                                registro_salidas, inmovih_table, saldo_inventory, dispatched_inventory,
                                inventario_sin_filtro]
    ...
    (wl_ingresos, rpshd_despachos, rpsdt_productos, registro_ingresos, registro_salidas, inmovih_table,
     saldo_inventory, dispatched_inventory, inventario_sin_filtro) = filtered_dataframes

    # Process and screen data
    (wl_ingresos, rpshd_despachos, rpsdt_productos, registro_ingresos, registro_salidas, inmovih_table,
     saldo_inventory, supplier_info, ctcentro_table, producto_modelos, dispatched_inventory,
     inventario_sin_filtro) = data_processing(wl_ingresos, rpshd_despachos, rpsdt_productos, registro_ingresos,
                                              registro_salidas, inmovih_table, saldo_inventory, supplier_info,
                                              ctcentro_table, producto_modelos, dispatched_inventory,
                                              inventario_sin_filtro)

    # Return processed data
    return {
        "wl_ingresos": wl_ingresos,
        "saldo_inventory": saldo_inventory,
...
def validate_dates(start_date_str, end_date_str):
    ...
    Args:
        start_date_str (str): Start date as string.
        end_date_str (str): End date as string.
...
def select_client(supplier_info):
    """
    Display and select a client.

    Args:
        supplier_info (pd.DataFrame): DataFrame with 'idcontacto' and 'descrip'.

    Returns:
        tuple: (entity_id, entity_name) of the selected client.
    """
    ...
    try:
        ...
        return selected_entity['idcontacto'], selected_entity['descrip']
    except ValueError:
...
def select_warehouse():
    """
    Display and select a warehouse.

    Returns:
        tuple: (entity_id, entity_name) of the selected warehouse or None for all warehouses.
    """
    ...
    try:
        ...
        if selected_idx == 'A':
            return None, "All Warehouses"
        selected_idx = int(selected_idx)
        if selected_idx < 0 or selected_idx >= len(warehouses):
            raise ValueError(f"Invalid selection '{selected_idx}'")
        return warehouses[selected_idx], warehouses[selected_idx]
    except ValueError:
        raise ValueError("Invalid input. Please enter a valid number.")


def filter_by_client(dataframes, client_id):
    """
    Filter multiple DataFrames by client ID.

    Args:
        dataframes (list): List of DataFrames to filter.
        client_id (str): Client ID to filter by.

    Returns:
        list: Filtered DataFrames.
    """
    return [df[df['idcontacto'] == client_id] for df in dataframes]

def filter_by_warehouse(dataframes, warehouse_id):
    """
    Filter multiple DataFrames by warehouse ID.

    Args:
        dataframes (list): List of DataFrames to filter.
        warehouse_id (str): Warehouse ID to filter by.

    Returns:
        list: Filtered DataFrames.
    """
    return [df[df['bodega'] == warehouse_id] for df in dataframes]


def generate_monthly_summaries(registro_ingresos, registro_salidas, supplier_info, dispatched_inventory):
    """
    Generate monthly summaries for receptions and dispatches.

    Args:
        registro_ingresos (pd.DataFrame): Inflow data.
        registro_salidas (pd.DataFrame): Outflow data.
        supplier_info (pd.DataFrame): Supplier info.
        dispatched_inventory (pd.DataFrame): Dispatched inventory.

    Returns:
        dict: Summary DataFrames.
    """
    resumen_receptions = monthly_receptions_summary(
        registro_ingresos, supplier_info, None, None
    )
    resumen_dispatches = monthly_dispatch_summary(
        registro_salidas, dispatched_inventory, supplier_info
    )
    return {
        "receptions": resumen_receptions,
        "dispatches": resumen_dispatches
    }


def analyze_inventory(saldo_inventory, supplier_info):
    """
    Perform inventory analysis.

    Args:
        saldo_inventory (pd.DataFrame): Inventory data.
        supplier_info (pd.DataFrame): Supplier info.

    Returns:
        dict: Analysis results (proportions, oldest products, etc.).
    """
    if saldo_inventory.empty or supplier_info.empty:
        raise ValueError("No inventory data available for analysis.")

    inventory_proportions = inventory_proportions_by_product(saldo_inventory, supplier_info)
    oldest_products = inventory_oldest_products(saldo_inventory, supplier_info)
    return {
        "proportions": inventory_proportions,
        "oldest_products": oldest_products
    }


def calculate_kpis(inventory_over_time, inventory_ot_by_month, start_date, end_date):
    """
    Calculate KPIs for analysis.

    Args:
        inventory_over_time (pd.DataFrame): Historical inventory data.
        inventory_ot_by_month (pd.DataFrame): Monthly inventory data.
        start_date (datetime): Start date.
        end_date (datetime): End date.

    Returns:
        dict: KPIs calculated.
    """
    return kpi_calculation(inventory_over_time, inventory_ot_by_month, start_date, end_date)
